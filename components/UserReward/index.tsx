...
function UserReward({ data }: Props) {
  let levelname = 'Newbie';
  let levelValueClass =
    'value level level' + (data.levelAccount - 1).toString();
      ...
      >
        ...
        <div className="row mb-4">
          ...
          <div className="level-upgrade col-md-6">
            ...
            <span className="level1" id="level-name">
              {levelname} Influencer
            </span>{' '}
            Reviewer. Want to level up to{' '}
            <span className="level2" id="level-next">
              {levelnames[data.levelAccount]} Influencer
            </span>
            ? Just write <strong id="review-privilege">{data.reviewNb}</strong>{' '}
        ...
        <div className="row">
          <div className="total col-12">
            In total, you got {data.totalReward} VeritaCoins (VT) since your
            registration. Well done! Want more rewards? Share your experience
            with the community and write a review.
          </div>
        </div>

        <div className="d-grid gap-2 col-6 mx-auto">
          <a className="btn btn-success d-block mt-2 text-white" href="#">
            Add new review
          </a>
        </div>
      </div>
