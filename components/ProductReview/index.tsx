    ...
    <>
      <div className="bg-white p-3 mb-2 rounded-5 shadow aos animated aos-init aos-animate" data-aos="fade-up"
        ...
        <div className="profil d-flex pb-2">
          <img alt="product review" src={'/img/star-0.jpg'} width="50" height="50"/>
          <div className="info ms-3">
            <p className="name lead mb-0">FIRSTNAME? LASTNAME? TODO: brk Fatah</p>
            <p className="analyse text-muted mb-0">NBR?? TODO reviews • <i className="flaticon-location"></i> FR</p>
          </div>
        </div>
        <div className="notation d-lg-flex justify-content-between my-3">
          ...
          <div className="date text-primary ms-auto">{moment(productReview.experienceDate).format("MMM DD, YYYY")}</div>
        </div>
        <div className="content mb-2">
          <p><span className="badge bg-primary">{productReview.productName}</span></p>
          <h3>{productReview.title} </h3>
          <p className="text-muted">{productReview.content}
          </p>
          <ul className="d-flex">

            {productReview.imageVideo && productReview.imageVideo.split(',').map((image, index) => (
              <li key={index}>
                <a href="#">
