...
function ServiceReview({merchantReview}: Props) {
  ...
  return (
    <>
      <div className="bg-white p-3 mb-2 rounded-5 shadow aos animated aos-init aos-animate" merchantReview-aos="fade-up"
           merchantReview-aos-anchor-placement="top-bottom">

        <div className="profil d-flex pb-2">
          <Image alt={'star image'} src={'/img/star-1.png'} width="50" height="50"/>
          <div className="info ms-3">
            <p className="name lead mb-0">{merchantReview.first_name} {merchantReview.last_name}</p>
            <p className="analyse text-muted mb-0">{merchantReview.Nbre} reviews • <i
              className="flaticon-location"></i> {merchantReview.location}</p>
          </div>
        </div>
        <div className="notation d-lg-flex justify-content-between my-3">
          <div className="note small me-lg-3 mb-2">
            <div className="form__star d-flex justify-content-start">
              <div><Image alt="Noté 5 sur 5 étoiles" src={srcimg} width="120" height="70"/></div>
            </div>
          </div>
          <div className="verified me-lg-3 mb-2">
            ...
            <button type="button" className="certified small bg-success text-white d-inline px-0 px-1 rounded-3"
                    merchantReview-bs-html="true" merchantReview-bs-trigger="focus" merchantReview-bs-toggle="popover"
          ...
          <div
            className="date text-primary ms-auto">{moment(merchantReview.experienceDate).format("MMM DD, YYYY")}</div>
        </div>
        <div className="content mb-2">
          <h3>{merchantReview.title} </h3>
          <p className="text-muted">{merchantReview.content}
          </p>
